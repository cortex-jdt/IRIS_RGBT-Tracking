...
# FINAL VERSION: Aligns images in the camera's native distorted space, then
# undistorts all resulting images as a final step to create a clean,
# geometrically correct dataset. Also saves undistorted versions of the original depth maps.
...
def parse_yaml_file(filepath):
    ...
    with open(filepath, 'r') as f:
        try:
            lines = f.readlines()
            # Handle header line more robustly
...
def warp_image_to_distorted_rgb(thermal_img, depth_img, K_rgb, D_rgb, K_thermal, D_thermal, R_thermal_to_rgb, t_thermal_to_rgb, depth_scale):
    """
    Helper function to warp a single thermal image to the original, distorted RGB viewpoint.
    """
    ...
    rgb_pixels_distorted = np.vstack((x_coords.ravel(), y_coords.ravel())).T.astype(np.float32)
    
    rgb_pixels_undistorted = cv2.undistortPoints(rgb_pixels_distorted, K_rgb, D_rgb, P=K_rgb)
    rgb_pixels_undistorted = rgb_pixels_undistorted.reshape(-1, 2).T
    rgb_pixels_undistorted = np.vstack((rgb_pixels_undistorted, np.ones(w*h)))

    depth_values = depth_img.ravel() * depth_scale
    
    points_3d_rgb = np.linalg.inv(K_rgb) @ rgb_pixels_undistorted * depth_values
    
    points_3d_thermal = R_rgb_to_thermal @ points_3d_rgb + t_rgb_to_thermal.reshape(3,1)
    
    projected_pixels_thermal, _ = cv2.projectPoints(points_3d_thermal.T, np.zeros(3), np.zeros(3), K_thermal, D_thermal)
    projected_pixels_thermal = projected_pixels_thermal.reshape(-1, 2)
...
def visualize_and_save_all(base_path, output_dir, R_left_to_rgb, t_left_to_rgb, R_right_to_rgb, t_right_to_rgb, rgb_data, left_data, right_data, common_images):
    """
    Saves aligned images by warping thermal to the distorted RGB space, then undistorting both as a final step.
    """
    print("\n--- Visualizing and Saving Final Alignment for All Common Images ---")
    ...
    for i, timestamp_key in enumerate(common_images):
        ...
        colorized_depth_filename = f"realsense_depth_colorized_image_{timestamp_key}.png"
        ...
        colorized_depth_path = os.path.join(base_path, 'realsense', 'depth_colorized', colorized_depth_filename)
        ...
        colorized_depth_img = cv2.imread(colorized_depth_path)

        if any(img is None for img in [rgb_img, left_thermal_img, right_thermal_img, depth_img, colorized_depth_img]):
            print(f"Warning: Could not load one or more images for timestamp {timestamp_key}. Skipping.")
            continue

        # --- MODIFIED: Save all intermediate depth products for analysis ---
        # 1. Undistort the raw depth map and its colorized version and save them
        undistorted_depth = cv2.undistort(depth_img, K_rgb, D_rgb)
        undistorted_colorized_depth = cv2.undistort(colorized_depth_img, K_rgb, D_rgb)
        cv2.imwrite(os.path.join(output_dir, 'undistorted_depth_raw', f'undistorted_depth_raw_{timestamp_key}.png'), undistorted_depth)
        cv2.imwrite(os.path.join(output_dir, 'undistorted_depth_colorized', f'undistorted_depth_colorized_{timestamp_key}.png'), undistorted_colorized_depth)

        # 2. Inpaint the raw, distorted depth map (this is used for alignment)
        hole_mask = (depth_img == 0).astype(np.uint8)
        inpainted_distorted_depth = cv2.inpaint(depth_img, hole_mask, 5, cv2.INPAINT_NS)
        
        # 3. Warp the thermal images to the distorted RGB space using the inpainted distorted depth
        aligned_left_distorted = warp_image_to_distorted_rgb(left_thermal_img, inpainted_distorted_depth, K_rgb, D_rgb, K_left, D_left, R_left_to_rgb, t_left_to_rgb, depth_scale)
        aligned_right_distorted = warp_image_to_distorted_rgb(right_thermal_img, inpainted_distorted_depth, K_rgb, D_rgb, K_right, D_right, R_right_to_rgb, t_right_to_rgb, depth_scale)

        # 4. As a final step, undistort both the original RGB and the newly aligned thermal images
        undistorted_rgb = cv2.undistort(rgb_img, K_rgb, D_rgb)
        final_aligned_left = cv2.undistort(aligned_left_distorted, K_rgb, D_rgb)
        final_aligned_right = cv2.undistort(aligned_right_distorted, K_rgb, D_rgb)

        # 5. Save all the final, clean output files
        cv2.imwrite(os.path.join(output_dir, 'undistorted_rgb', f'undistorted_rgb_{timestamp_key}.png'), undistorted_rgb)
        cv2.imwrite(os.path.join(output_dir, 'undistorted_aligned_left_thermal', f'aligned_left_{timestamp_key}.png'), final_aligned_left)
        cv2.imwrite(os.path.join(output_dir, 'undistorted_aligned_right_thermal', f'aligned_right_{timestamp_key}.png'), final_aligned_right)
...
def find_validation_image_sets(base_path):
    ...
    colorized_depth_timestamps = get_timestamps_from_dir(os.path.join(base_path, 'realsense', 'depth_colorized'))

    if not all([rgb_timestamps, left_timestamps, right_timestamps, depth_timestamps, colorized_depth_timestamps]):
        print("Error: One or more image directories are empty or missing. Cannot find validation sets.")
        return []
    ...
    common_validation_stamps = sorted(list(
        rgb_timestamps.intersection(left_timestamps)
                      ...
                      .intersection(colorized_depth_timestamps)
    ))
...
def main():
    ...
    os.makedirs(os.path.join(output_dir, 'undistorted_aligned_left_thermal'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'undistorted_aligned_right_thermal'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'undistorted_depth_raw'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'undistorted_depth_colorized'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'undistorted_inpainted_depth_raw'), exist_ok=True)
    os.makedirs(os.path.join(output_dir, 'undistorted_inpainted_depth_colorized'), exist_ok=True)
